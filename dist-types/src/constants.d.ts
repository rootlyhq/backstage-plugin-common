/** @public */
export declare const ROOTLY_ANNOTATION_ORG_ID = "rootly.com/organization-id";
/** @public */
export declare const ROOTLY_ANNOTATION_SERVICE_ID = "rootly.com/service-id";
/** @public */
export declare const ROOTLY_ANNOTATION_SERVICE_NAME = "rootly.com/service-name";
/** @public */
export declare const ROOTLY_ANNOTATION_SERVICE_SLUG = "rootly.com/service-slug";
/** @public */
export declare const ROOTLY_ANNOTATION_SERVICE_AUTO_IMPORT = "rootly.com/service-auto-import";
/** @public */
export declare const ROOTLY_ANNOTATION_FUNCTIONALITY_NAME = "rootly.com/functionality-name";
/** @public */
export declare const ROOTLY_ANNOTATION_FUNCTIONALITY_ID = "rootly.com/functionality-id";
/** @public */
export declare const ROOTLY_ANNOTATION_FUNCTIONALITY_SLUG = "rootly.com/functionality-slug";
/** @public */
export declare const ROOTLY_ANNOTATION_FUNCTIONALITY_AUTO_IMPORT = "rootly.com/functionality-auto-import";
/** @public */
export declare const ROOTLY_ANNOTATION_TEAM_ID = "rootly.com/team-id";
/** @public */
export declare const ROOTLY_ANNOTATION_TEAM_NAME = "rootly.com/team-name";
/** @public */
export declare const ROOTLY_ANNOTATION_TEAM_SLUG = "rootly.com/team-slug";
/** @public */
export declare const ROOTLY_ANNOTATION_TEAM_AUTO_IMPORT = "rootly.com/team-auto-import";
/** @public */
export declare const ROOTLY_ANNOTATION_CATALOG_ENTITY_ID = "rootly.com/catalog-entity-id";
/** @public */
export declare const ROOTLY_ANNOTATION_CATALOG_ENTITY_NAME = "rootly.com/catalog-entity-name";
/** @public */
export declare const ROOTLY_ANNOTATION_CATALOG_ENTITY_SLUG = "rootly.com/catalog-entity-slug";
/** @public */
export declare const ROOTLY_ANNOTATION_CATALOG_ENTITY_AUTO_IMPORT = "rootly.com/catalog-entity-auto-import";
/** @public */
export declare const ROOTLY_ANNOTATION_CATALOG_ID = "rootly.com/catalog-id";
/** @public */
export declare const ROOTLY_ANNOTATION_CATALOG_SLUG = "rootly.com/catalog-slug";
