...
function extractAnnotationEntries(annotations, prefix) {
  if (!annotations) return [];
  return Object.entries(annotations).filter(([k]) => k.startsWith(prefix)).map(([k, v]) => [k.slice(prefix.length), v]).filter(([k]) => k !== "");
}
function coerceAnnotationValue(value) {
...
class RootlyApi {
  ...
  async importServiceEntity(entity) {
    ...
    const passthroughAttrs = extractPassthroughAttributes(entity.metadata.annotations, constants.ROOTLY_ANNOTATION_SERVICE_ATTR_PREFIX);
    const properties = extractProperties(entity.metadata.annotations, constants.ROOTLY_ANNOTATION_SERVICE_PROPERTY_PREFIX);
    const init = {
      method: "POST",
  ...
  async importFunctionalityEntity(entity) {
    ...
    const passthroughAttrs = extractPassthroughAttributes(entity.metadata.annotations, constants.ROOTLY_ANNOTATION_FUNCTIONALITY_ATTR_PREFIX);
    const init = {
      method: "POST",
      headers: { "Content-Type": "application/vnd.api+json" },
      body: JSON.stringify({
  ...
  async importTeamEntity(entity) {
    ...
    const passthroughAttrs = extractPassthroughAttributes(entity.metadata.annotations, constants.ROOTLY_ANNOTATION_TEAM_ATTR_PREFIX);
    const init = {
      method: "POST",
      headers: { "Content-Type": "application/vnd.api+json" },
      body: JSON.stringify({
  ...
  async getCatalogEntity(id_or_slug, opts) {
    const init = { headers: { "Content-Type": "application/vnd.api+json" } };
    const params = opts ? qs__default.default.stringify(opts, { encode: false }) : "";
    const response = await this.fetch(
  ...
  async importCatalogEntityEntity(entity, catalogId) {
    ...
    const passthroughAttrs = extractPassthroughAttributes(entity.metadata.annotations, constants.ROOTLY_ANNOTATION_CATALOG_ENTITY_ATTR_PREFIX);
    const properties = extractProperties(entity.metadata.annotations, constants.ROOTLY_ANNOTATION_CATALOG_ENTITY_PROPERTY_PREFIX);
    const init = {
      method: "POST",
      headers: { "Content-Type": "application/vnd.api+json" },
      body: JSON.stringify({
...
exports.extractPassthroughAttributes = extractPassthroughAttributes;
